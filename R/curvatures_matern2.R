...
curvatures_matern2 <- nimble::nimbleFunction(
  run = function(dists.1 = double(2),
    ...
    for(i in 1:nmcmc){
      Sigma[1:ncoords, 1:ncoords] <- materncov2(dists = dists.1[1:ncoords,1:ncoords],
                                                phi = phi[i],
                                                sigma2 = 1,
                                                tau2 = 0)

      for(j in 1:ngrid){
        nabla.K[1:ncoords, 1] <- -5/3 * phi[i]^2 * (1 + sqrt(5) * phi[i] * dists.2[1:ncoords, j]) * exp(-phi[i] * sqrt(5) * dists.2[1:ncoords, j]) * dists.3[(2 * j - 1), 1:ncoords]
        nabla.K[1:ncoords, 2] <- -5/3 * phi[i]^2 * (1 + sqrt(5) * phi[i] * dists.2[1:ncoords, j]) * exp(-phi[i] * sqrt(5) * dists.2[1:ncoords, j]) * dists.3[2 * j, 1:ncoords]
        nabla.K[1:ncoords, 3] <- -5/3 * phi[i]^2 * (1 + sqrt(5) * phi[i] * dists.2[1:ncoords, j] - 5 * phi[i]^2* dists.3[(2 * j - 1), 1:ncoords]^2) * exp(-phi[i] * sqrt(5) * dists.2[1:ncoords, j])
        nabla.K[1:ncoords, 4] <- 25/3 * phi[i]^2 * exp(-phi[i] * sqrt(5) * dists.2[1:ncoords, j]) * dists.3[(2 * j - 1), 1:ncoords] * dists.3[2 * j, 1:ncoords]
        nabla.K[1:ncoords, 5] <- -5/3 * phi[i]^2 * (1 + sqrt(5) * phi[i] * dists.2[1:ncoords, j] - 5 * phi[i]^2* dists.3[2 * j, 1:ncoords]^2) * exp(-phi[i] * sqrt(5) * dists.2[1:ncoords, j])
        V0[1, 1] <- 5/3 * phi[i]^2
        V0[2, 2] <- 5/3 * phi[i]^2
        V0[3, 3] <- 25 * phi[i]^4
        V0[3, 5] <- 25/3 * phi[i]^4
